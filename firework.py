...
def simulate(cv):
    t = time()
    explode_points = []
    wait_time = randint(10,150)
    numb_explode = randint(1,7)
    # create list of list of all particles in all simultaneous explosion
    for point in range(numb_explode):
        objects = []
        x_cordi = randint(10,250)
        y_cordi = randint(50, 100)
        speed = uniform (0.5, 1.5)          
        size = uniform (0.5,2.5)
        color = choice(colors)
        explosion_speed = uniform(0.2, 1.0)
...
if __name__ == '__main__':
    root = tk.Tk()
    cv = tk.Canvas(root, height=150, width=300)
    # use a nice background image
    image = Image.open("C:\chen2\download.jpg")
